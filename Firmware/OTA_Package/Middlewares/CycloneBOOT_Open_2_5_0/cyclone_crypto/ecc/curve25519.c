...
void curve25519Import(int32_t *a, const uint8_t *data)
{
   ...
   a[1] |= (temp << 3) & 0x1FFFFFF8;
   a[2] = temp >> 26;
   temp = LOAD32LE(data + 8);
   a[2] |= (temp << 6) & 0x1FFFFFC0;
   a[3] = temp >> 23;
   temp = LOAD32LE(data + 12);
   a[3] |= (temp << 9) & 0x1FFFFE00;
   a[4] = temp >> 20;
   temp = LOAD32LE(data + 16);
   a[4] |= (temp << 12) & 0x1FFFF000;
   a[5] = temp >> 17;
   temp = LOAD32LE(data + 20);
   a[5] |= (temp << 15) & 0x1FFF8000;
   a[6] = temp >> 14;
   temp = LOAD32LE(data + 24);
   a[6] |= (temp << 18) & 0x1FFC0000;
   a[7] = temp >> 11;
   temp = LOAD32LE(data + 28);
   a[7] |= (temp << 21) & 0x1FE00000;
   a[8] = temp >> 8;
}
