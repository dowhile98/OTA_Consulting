...
void curve448Import(int32_t *a, const uint8_t *data)
{
   ...
   a[1] |= (temp << 4) & 0x0FFFFFF0;
   a[2] = temp >> 24;
   temp = LOAD32LE(data + 8);
   a[2] |= (temp << 8) & 0x0FFFFF00;
   a[3] = temp >> 20;
   temp = LOAD32LE(data + 12);
   a[3] |= (temp << 12) & 0x0FFFF000;
   a[4] = temp >> 16;
   temp = LOAD32LE(data + 16);
   a[4] |= (temp << 16) & 0x0FFF0000;
   a[5] = temp >> 12;
   temp = LOAD32LE(data + 20);
   a[5] |= (temp << 20) & 0x0FF00000;
   a[6] = temp >> 8;
   temp = LOAD32LE(data + 24);
   a[6] |= (temp << 24) & 0x0F000000;
   a[7] = temp >> 4;
   ...
   a[9] |= (temp << 4) & 0x0FFFFFF0;
   a[10] = temp >> 24;
   temp = LOAD32LE(data + 36);
   a[10] |= (temp << 8) & 0x0FFFFF00;
   a[11] = temp >> 20;
   temp = LOAD32LE(data + 40);
   a[11] |= (temp << 12) & 0x0FFFF000;
   a[12] = temp >> 16;
   temp = LOAD32LE(data + 44);
   a[12] |= (temp << 16) & 0x0FFF0000;
   a[13] = temp >> 12;
   temp = LOAD32LE(data + 48);
   a[13] |= (temp << 20) & 0x0FF00000;
   a[14] = temp >> 8;
   temp = LOAD32LE(data + 52);
   a[14] |= (temp << 24) & 0x0F000000;
   a[15] = temp >> 4;
}
...
void curve448Export(int32_t *a, uint8_t *data)
{
   ...
   STORE32LE(temp, data);
   temp = (a[2] << 24) | (a[1] >> 4);
   STORE32LE(temp, data + 4);
   temp = (a[3] << 20) | (a[2] >> 8);
   STORE32LE(temp, data + 8);
   temp = (a[4] << 16) | (a[3] >> 12);
   STORE32LE(temp, data + 12);
   temp = (a[5] << 12) | (a[4] >> 16);
   STORE32LE(temp, data + 16);
   temp = (a[6] << 8) | (a[5] >> 20);
   STORE32LE(temp, data + 20);
   temp = (a[7] << 4) | (a[6] >> 24);
   STORE32LE(temp, data + 24);

   temp = (a[9] << 28) | a[8];
   STORE32LE(temp, data + 28);
   temp = (a[10] << 24) | (a[9] >> 4);
   STORE32LE(temp, data + 32);
   temp = (a[11] << 20) | (a[10] >> 8);
   STORE32LE(temp, data + 36);
   temp = (a[12] << 16) | (a[11] >> 12);
   STORE32LE(temp, data + 40);
   temp = (a[13] << 12) | (a[12] >> 16);
   STORE32LE(temp, data + 44);
   temp = (a[14] << 8) | (a[13] >> 20);
   STORE32LE(temp, data + 48);
   temp = (a[15] << 4) | (a[14] >> 24);
   STORE32LE(temp, data + 52);
}
