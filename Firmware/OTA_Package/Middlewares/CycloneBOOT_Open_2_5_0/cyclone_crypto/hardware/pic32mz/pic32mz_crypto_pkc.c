   ...
   const uint8_t *k, const Ed25519Point *p)
{
   ...
   //Write the input point into the SCM memory
   ba414eImportScalar(p->x, 8, BA414E_ED25519_SLOT_PX);
   ba414eImportScalar(p->y, 8, BA414E_ED25519_SLOT_PY);
   ...
   //Read the resulting point from the SCM memory
   ba414eExportScalar(r->x, 8, BA414E_ED25519_SLOT_CX);
   ba414eExportScalar(r->y, 8, BA414E_ED25519_SLOT_CY);
   ...
   const Ed25519Point *q)
{
   ...
   //Compute R = k1 * P + k2 * Q
   ed25519Mul(state, &p1, k1, p);
   ed25519Mul(state, &p2, k2, q);
