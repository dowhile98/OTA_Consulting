...
const FlashInfo stm32h5xxFlashDriverInfo = {
...
error_t stm32h5xxFlashDriverWrite(uint32_t address, uint8_t* data, size_t length)
{
   ...
   if(data == NULL ||
      !((address >= STM32H5xx_FLASH_BANK1_ADDR && address + length <= STM32H5xx_FLASH_BANK2_ADDR) ||
...
error_t stm32h5xxFlashDriverErase(uint32_t address, size_t length)
{
   ...
   if((length == 0) || !((address >= STM32H5xx_FLASH_BANK1_ADDR && address + length <= STM32H5xx_FLASH_BANK2_ADDR) ||
         (address >= STM32H5xx_FLASH_BANK2_ADDR && address + length <= topAddress)))
      return ERROR_INVALID_PARAMETER;
#endif
   ...
   firstSectorNumber = stm32h5xxFlashGetSector(address);

   if(address + length == topAddress)
   {
      //Set last sector number as the flash sector total number
      lastSectorNumber = STM32H5xx_FLASH_SECTOR_NUMBER;
   }
   else
   {
      //Get the address of the boundary sector (not to be erased)
      error = stm32h5xxFlashDriverGetNextSector(address+length, &lastSectorAddr);
      //Is any error?
      if(error)
         return error;

      //The last sector to erase is the last flash bank 1 sector
      lastSectorNumber = stm32h5xxFlashGetSector(lastSectorAddr);
   }

   //Erase sectors of the flash bank 1 (the only one flash bank)
   error = stm32h5xxFlashDriverEraseSector(STM32H5xx_FLASH_BANK1_ID, firstSectorNumber, lastSectorNumber-firstSectorNumber);
   //Is any error?
   if(error)
      return error;
   ...
   if(address < STM32H5xx_FLASH_BANK2_ADDR)
   {
      //Get the number of the first sector to erase
      firstSectorNumber = stm32h5xxFlashGetSector(address);

      //Is there data to erase in second flash bank?
      if(address + length > STM32H5xx_FLASH_BANK2_ADDR)
      {
         //Set last sector number as the first flash bank 2 sector number
         lastSectorNumber = STM32H5xx_FLASH_SECTOR_NUMBER/2;
   ...
   if(length && address >= STM32H5xx_FLASH_BANK2_ADDR)
   {
      //Get the number of the first sector to erase
      firstSectorNumber = stm32h5xxFlashGetSector(address);
      ...
      error = stm32h5xxFlashDriverEraseSector(STM32H5xx_FLASH_BANK2_ID, firstSectorNumber, lastSectorNumber-firstSectorNumber);
      //Is any error?
      if(error)
         return error;
   }
...
#else
   //Check parameters validity (is data in flash bank 1 or 2 only)
   if(!((address >= STM32H5xx_FLASH_BANK1_ADDR && address + sizeof(uint32_t) <= STM32H5xx_FLASH_BANK2_ADDR) ||
...
error_t stm32h5xxFlashDriverEraseSector(uint32_t bankID, uint32_t firstSector, size_t nbSectors)
{
   ...
   do
   {
      ...
      {
         //Debug message
         TRACE_ERROR("Failed to erase flash sector(s) %lu, error = 0x%08lx!\r\n", firstSector, sectorError);
      }
...
/**
 * @brief Get current used flash bank (STM32H5xx_FLASH_BANK1_ID or STM32H5xx_BANK_2_ID).
